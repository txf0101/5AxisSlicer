...
新建桌面项目默认使用“自有 AC 五轴打印机 / Own AC FDM”。点击顶部“公共制造设置”，在左侧树选择“机床（Machine）”，右侧型号栏可切换机型。该设置供各制造工作台共用。打开旧项目时保留项目机型；新项目使用自有机型默认值。

## 选择机型

从型号栏选择自有 AC、Cartesian Reference、Generic XYZAC Reference 或用户库机型，再点击“应用”。只浏览下拉列表不会修改已应用配置。切换机型后装夹和相关生成结果需要重新检查。

![公共制造设置中的机型选择、应用和轴字设置入口](assets/product_delivery/08_machine_setup_zh.png)
...
保存副本后，可在机型下拉框中选中它，再检查左侧树的“机床（Machine）”是否显示“有效”。修改配置后，相关操作须重新生成。

## 旋转轴输出字：A/C 改为 U/W

内部轴名用于运动学和轴轨迹，控制器输出字用于 `main.gcode`。例如自有 AC 机型可保留内部 A/C，同时让固件接收 U/W：

1. 在顶部“公共制造设置”的“机床（Machine）”页选中目标机型。
2. 点击“旋转轴 G-code 输出字…”。
...
当前接受单个 ASCII 字母并自动转大写。输出字不能与其他关节重复，也不能使用 `E/F/G/M/N/P/S/T`：这些字在当前生成方言或控制器命令中已有固定含义。非法输入不会修改已应用机型。允许的自定义字可被生成结果的严格 G-code 回读识别；“G-code 文件预览”只用于查看已有程序，不能代替生成结果的严格回读。
