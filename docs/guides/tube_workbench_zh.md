...
真实弯管已完成底座与三道管壁的整件离线 NC、208232 点全流回读。公共 Tube 产品链会按已审核材料配置加入开机预热与结束关热；包装前后运动文本须一致，仍需核对逐层承接。碰撞检查按用户要求单独暂缓，不能据此认定实机可打印。证据和保留事项见 [FAN15 修复记录](../reviews/2026-09-20_fan15_repairs.md)。
...
旧的界面示例六件套保存在 `docs/reviews/evidence/2026-09-13_paper_core_ac/products/pipe2/`。2026-09-21 的整件离线 NC 在 `docs/reviews/evidence/2026-09-20_fan15_repairs/pipe_offline_nc_v5_20260921/`；这套诊断生成显式暂缓了 IPW 碰撞检查，须查看同目录检查范围。参数与点数是本案例数据，不是所有模型的保证值。
