...
本指南说明 Tube 工作台的制造设置与坐标编辑器。当前只有**管状工作台**接入完整制造设置、坐标与生成闭环；平面、曲线、自由曲面和回转工作台仍进入通用操作会话，那里没有“模型坐标系”“构建坐标系”和“装夹定位”节点。
...
- “使用 R–Z 碰撞外形（缺失时生成简化外形）”会保留 Profile 已有外形；模板没有外形时，程序根据喷孔直径和总长生成粗略轴对称轮廓。该数据用于资源完整性和后续碰撞建模预留；当前版本不渲染该外形，也不执行碰撞检查。缺少实物几何依据时，不能据此判断防碰撞或 NC 安全性。
