...
判断模型是否适合 Rotary 时，要看所选区域能否由一条固定轴和同轴圆柱/圆锥面表达。带弯头的扫掠管即使外观具有回转特征，局部轴也可能沿中心线变化；这种区域应按实际中心线和截面选择其他工作台。
...
当前 Generic XYZAC 只提供离线参考资格。自定义轴字会写入 G-code，并参与回读检查；目标固件对轴字的物理含义仍需按设备定义核对。回转正方向、单位、缩放、零偏、轴速度和加速度、回转中心、工具长度、实际喷嘴包络、机床壳体、现场夹具和材料参数均应按目标机床调整。实际参数、实际效果以用户实际调试为准，本项目持续完善中。
