...
| FAN11 | 作业GUI和三入口持久化 | FAN10 | 实施中 | GUI/脚本/HTTP共核、后台取消、重开和依赖失效 | 四例底层算法与离线 NC 已完成；新增实体填充仍由 finalizer 编排，尚未进入公共操作契约；[产品化计划](fan15_productization_plan.md) | FP01 操作契约 | 2026-09-21 |
| FAN12 | 电脑真实用户点击验收 | FAN11 | 等待 FAN11 | UI01—UI12、双语三尺寸、GUI生成文件hash | 弯管真实加载及四角色拾取已有证据；其余三例未走通，最近 Computer Use 应用枚举失败；[产品化计划](fan15_productization_plan.md) | FAN11 完成后执行 Qt 与真实点击矩阵 | 2026-09-21 |
...
| FAN15 | 其他示例逐件迁移 | FAN14 | 四例离线 NC 已验收；产品化回迁中 | 叶轮/管件/半球/三叶扇逐例子项和判据 | [四例审查](../reviews/2026-09-20_fan15_example_acceptance.md)；[修复过程](../reviews/2026-09-20_fan15_repairs.md)。四例 OFFLINE NC、严格回读、独立逆变换和对比图通过；新增三类填充算法在 `src/`，完整编排仍在 finalizer；[产品化计划](fan15_productization_plan.md) | 按 FP01—FP06 接入公共命令、UI、重开和教程；实机/IPW 单列 | 2026-09-21 |
