...
2026-09-13最新路线为[论文核心AC范围与复用研究](paper_core_ac_scope.md#4-本轮开源资料核查与采用决定)。该专项记录7个仓库的当前固定commit、README/关键入口和许可证核查，以及半球/扇叶/弯管/叶轮能力差距与论文批判性审查。现有OCP/OCCT优先；Open5x、COMPAS Slicer为MIT部件候选；Clipper2为Boost许可备选；Klipper/Happy Hare仅先作宏接口参考，GPL集成义务另审；MAGE含Noncompete，本版不移植。COMPAS的compas_cgal依赖实读为LGPLv3，不能随主项目一并称MIT。未安装/执行上游程序或引入第三方实现。

下文场优化、应力路径和通用多机型来源保留供长期路线使用，当前不为这些延期任务消耗实施资源。Siemens公开产品页/2506说明本轮返回200，仍不代表取得私有NX手册正文或算法源码。
