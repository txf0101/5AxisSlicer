...
OPEN5X_PROFILE_DESCRIPTION = (
    "Converted Cartesian printer with a rotary bed. The first rotary axis tilts the bed around the machine Y axis. "
    "The second rotary axis spins the already tilted bed around its local rotary axis."
)
OPEN5X_PROFILE_DESCRIPTION_ZH = (
    "笛卡尔三轴打印机加装双旋转打印床。第一旋转轴让床面绕机器 Y 轴倾斜，"
    "第二旋转轴让已经倾斜的床面继续绕局部旋转轴转动。"
)
...
def open5x_freddi_hong_machine() -> MachineParameters:
    ...
    return MachineParameters(
        ...
        rotary_safe_z_mm=150.0,
        rotary_safe_reposition_trigger_deg=25.0,
        u_axis_sign=1,
        v_axis_sign=1,
...
def machine_profile_summary(machine: MachineParameters, language: str = "en") -> str:
    ...
    if language == "zh":
        return (
            f"{machine.profile_name}：{machine.u_axis_name} 轴绕机器 Y 轴倾斜，"
            f"{machine.v_axis_name} 轴绕倾斜后的局部轴转动，"
    ...
    return (
        f"{machine.profile_name}: {machine.u_axis_name} tilts around machine Y, "
        f"{machine.v_axis_name} spins around the local tilted axis, "
